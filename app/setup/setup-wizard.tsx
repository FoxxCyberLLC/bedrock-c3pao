...
import { validateInstanceKey, completeSetup, completeOfflineSetup } from '@/app/actions/setup'
...
/**
 * First-run setup wizard. In air-gapped mode (`offline`) the connection/validation step is
 * skipped entirely — the operator names the C3PAO and creates the local admin, and setup
 * completes with no network call (`completeOfflineSetup`).
 */
export function SetupWizard({ offline }: { offline: boolean }) {
  const [step, setStep] = useState<WizardStep>('welcome')
  const [apiUrl, setApiUrl] = useState('')
  const [apiKey, setApiKey] = useState('')
  const [c3paoName, setC3paoName] = useState('')
  ...
  async function handleComplete() {
    if (!offline && !activationData) return

    if (offline && !c3paoName.trim()) {
      setError('Please enter the C3PAO organization name')
      return
    }
    if (!adminName.trim()) {
    ...
    try {
      const result = offline
        ? await completeOfflineSetup({
            c3paoId: crypto.randomUUID(),
            c3paoName: c3paoName.trim(),
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-lg">
        ...
        {step === 'welcome' && (
          <Card>
            <CardHeader className="text-center">
              ...
              <CardDescription>
                {offline
                  ? 'This is an air-gapped assessor instance. Name your C3PAO organization and create a local admin account — no network connection is required.'
                  : "Configure this instance to connect with the Bedrock CMMC platform. You'll need the API URL, the instance key from your Bedrock administrator, and create a local admin account."}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button onClick={() => setStep(offline ? 'admin' : 'connection')} className="w-full" size="lg">
                Get Started
        ...
        {/* Admin account step */}
        {step === 'admin' && (offline || activationData) && (
          <Card>
            <CardHeader className="text-center">
              <div className="mx-auto w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mb-4">
            ...
            <CardContent className="space-y-5">
              {/* Verified organization banner (online) */}
              {!offline && activationData && (
              ...
              {/* C3PAO organization name (air-gapped) */}
              {offline && (
                <div className="space-y-2">
                  <label htmlFor="c3paoName" className="text-sm font-medium">
                    C3PAO Organization Name
                  </label>
                  <div className="relative">
                    ...
                    <Input
                      id="c3paoName"
                      type="text"
                      placeholder="Number One Cyber"
                      value={c3paoName}
                      onChange={(e) => {
                        setC3paoName(e.target.value)
                        setError('')
                      }}
                      className="pl-10"
                    />
              ...
              <div className="flex gap-3">
                <Button
                  variant="outline"
                  onClick={() => {
                    setStep(offline ? 'welcome' : 'connection')
