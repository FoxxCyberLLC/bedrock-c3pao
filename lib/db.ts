...
import { ensureOrgSchema } from './local/coi-qa'
...
export async function ensureSchema(): Promise<void> {
  ...
  _schemaPromise = (async () => {
    ...
    // Raw imported-snapshot tables (imp_*) for offline OSC engagements.
    await ensureImportedSchema(pool)

    // C3PAO-local collaboration tables (notes / check-ins / comments).
    ...
    // C3PAO-local COI disclosures + QA reviews.
    await ensureOrgSchema(pool)
